...
- **`Motherboard` scos complet din aplicaţie** — 2026-09-05, la cererea lui Gabriel. Era un tab din `AdminDashboard` (sidebar desktop + bottom-nav mobil), 1490 de linii, dar fără nicio interogare reală către Supabase — tot ce afişa era date hardcodate (`CHIMPEX_DATA` etc.), nu date live. Scos importul şi wiring-ul din `AdminDashboard/index.jsx` şi `AdminSidebar.jsx`, şters `src/pages/Motherboard.jsx` + `src/pages/Motherboard/StocksPage.jsx`, adăugat un fallback pentru cazul în care un admin avea "motherboard" salvat ca ultimul tab vizitat în `localStorage` (altfel ar fi rămas cu ecranul gol). Verificat cu `npm run lint` + `npm run build` după. Ruta veche `/motherboard` din `App.jsx` rămâne — oricum doar redirecta spre `/dashboard`, nu afişa nimic. CSS-ul mort rămas (`.mb-*`, ~80 de linii în `styles.css`) nu a fost curăţat — ar necesita verificare atentă într-un fişier CSS deja monolitic, separat de cererea de azi.
...
- **Fişiere monolitice:** `BidsTab.jsx` (827 linii), `ActivityTab.jsx` (683 linii). `Motherboard.jsx` (1490 linii) — scos complet din aplicaţie 2026-09-05, vezi rezumatul din secţiunea 1.
