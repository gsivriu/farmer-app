...
- [ ] TESTFLIGHT LOGIN LOOP, re-reported 2026-09-13 — **the code fix was never the problem the second time; the device was running a stale bundle.** Prod's deployed bundle (`index-Z13zX47Z.js`, live since 2026-09-10 12:22 UTC, `47ff0d8`) provably contains both the fix and the `auth_debug_logs` instrumentation, and `/` is served `Cache-Control: no-store`. Yet the 2026-09-13 11:30 UTC prod edge logs show the phone doing a full login round-trip (`token?grant_type=password` 200 → `profiles` 200 → `logout?scope=local` 204, 1.3s later, twice) with **zero** `POST /rest/v1/auth_debug_logs` requests anywhere in the sequence — impossible for that bundle, which writes a row on every `onAuthStateChange` event. `auth_debug_logs`' newest row is still 2026-09-10 12:19:59. So the device has been executing pre-`cdbfa58` JS, i.e. the original broken `useIdleLogout`, which is exactly why the symptom is unchanged. Two candidate mechanisms, both fixed the same way (get new code onto the device): the TestFlight build runs assets frozen into the IPA (`capacitor.config.json` has no `server` block → bundled, so no web deploy can ever reach it — needs `npm run build && npm run cap:sync:ios` + a new TestFlight build), or it is in `cap:live` mode and WKWebView is serving an `index.html` it cached *before* the `no-store` header shipped (needs a delete-and-reinstall to drop that cache). **Lesson: "is the fix deployed?" and "is the device running the fix?" are different questions, and before 2026-09-13 nothing in the app could answer the second one** — hence the build stamp below.
