...
`useIdleLogout` (`src/hooks/useIdleLogout.js`, wired in via `useAuth.jsx`, enabled whenever `user` is truthy) signs out any session — farmer or admin, web or TestFlight — after 1 hour with no interaction, counting backgrounded/locked time too so a phone left locked with the app open still gets caught on resume. It reads/writes a `farmer-app-last-active-at` timestamp in `localStorage`. `markIdleActivity()` (same file) resets that timestamp and **must** be called on the `SIGNED_IN` auth event specifically (not on `INITIAL_SESSION`/session restore) — see the Known Issues entry below for why.
...
- `gnews` also keeps a **shared server-side cache** (`news_cache` table, migration `20260913120000_news_cache.sql`, 30 min TTL) — added 2026-09-13 after the News tab started failing with GNews' *"too many requests in a short period"*. News is identical for every user, but each tab visit used to trigger its own upstream call, so the free GNews plan's daily quota and burst protection were trivially exhausted — and at the 5,000-user target it was O(users × visits) upstream calls for one piece of content. Now it's O(1 per TTL). An expired row is deliberately kept: when GNews rate-limits or is down, the function serves the stale payload with `X-Cache: stale` instead of an error, so the tab degrades to slightly old news rather than breaking. `news_cache` has RLS on with **no policies** — only `service_role` touches it, never the client. Client side, `useNewsData` holds a 15-min module-level cache and collapses concurrent calls into one in-flight request (two near-simultaneous mounts used to fire two real upstream calls, which is what tripped the burst limit).
...
- [ ] TESTFLIGHT LOGIN LOOP, re-reported 2026-09-13 — **settled: the TestFlight app runs web assets frozen at build time and has never executed any fix shipped since 2026-09-10.** The deployed prod bundle (`index-Z13zX47Z.js`, live since 2026-09-10 12:22 UTC) contains both the fix and the `auth_debug_logs` instrumentation, which writes a row on *every* `onAuthStateChange` event. Yet `auth_debug_logs` has **zero** rows for all of 2026-09-13 — including after Gabriel deleted and reinstalled the app and logged in successfully. A successful login necessarily fires `SIGNED_IN`, so the code that ran cannot be the deployed bundle. iOS wipes the app container (caches included) on delete, so a surviving WKWebView cache is ruled out: the app is loading code baked into the IPA — either bundled assets (`capacitor.config.json` has no `server` block) or `cap:live` pinned to a deployment-specific URL rather than the stable alias. Either way the web deploy can never reach it and **only a new TestFlight build (`npm run build && npm run cap:sync:ios`, then upload) fixes it.**
  - The reinstall appeared to fix login, but only because it wiped `localStorage` and with it the stale `farmer-app-last-active-at` stamp — the old code reads `Date.now()` when the key is absent, so the first login passes. **The loop will return** the first time the app sits closed for over an hour: the old `useIdleLogout` signs out on open and leaves the expired stamp behind (clearing it is the `80bc031` fix the device doesn't have), so the next login is bounced again.
  - This also explains why the 2026-09-09 fix looked verified: the debug rows from 2026-09-10 12:19 came from mobile Safari on the phone (which does fetch from Vercel), not from the TestFlight app. Every fix since has been verified against the web app while the actual app kept running pre-2026-09-10 code. **"We fixed it" and "it's still broken" were both true the whole time.**
  - Correction to `80bc031`'s commit message, which claimed hashed assets are served `immutable/max-age=1y`: checked 2026-09-13, they are served `public, max-age=0, must-revalidate`, as is the SPA-rewrite path for `index.html` (`/` itself is `no-store`). The stale-HTML-pinning mechanism that commit describes is therefore not what happened here.
- Build stamp (added 2026-09-13): `vite.config.js` injects the short commit SHA (`VERCEL_GIT_COMMIT_SHA`, else `git rev-parse`) and build time as `import.meta.env.APP_BUILD_ID`/`APP_BUILD_TIME`; `src/buildInfo.js` exposes them; the login screen renders `<sha> · <dd.mm>` in muted 11px text, and every `auth_debug_logs` row carries `detail.buildId`. Read it off the device before debugging any "the fix didn't work" report — if the stamp isn't the commit you expect, the device never got the code and nothing else matters.
- `useIdleLogout` structural guard (added 2026-09-13): `checkIdle()` now returns early when the session's `user.last_sign_in_at` is newer than `IDLE_TIMEOUT_MS`, because a session that authenticated less than an hour ago cannot have been idle for an hour. `markIdleActivity()` on `SIGNED_IN` and clearing the stamp at sign-out both fix the loop by keeping the stored timestamp *correct*; this guard holds even when it isn't, so no future stale-`localStorage` path can bounce a fresh login again.
- [ ] `auth_debug_logs` is temporary prod debug infrastructure with an `anyone_can_insert_auth_debug_logs` policy granting INSERT to `anon` *and* `authenticated` with `WITH CHECK (true)` — anyone holding the (public) anon key can write unbounded rows into prod. Acceptable only while the login loop is being diagnosed. **Drop the table, the policy, `logAuthDebug` in `useAuth.jsx` and the two inserts in `useIdleLogout.js` as soon as a device confirms a good build stamp + a clean login.**
- [x] ~~TESTFLIGHT LOGIN LOOP, reported 2026-09-09: enter credentials, press login, briefly authenticated, then bounced straight back to `/login`~~ — caused by `useIdleLogout` (added 2026-09-05, `54fd6a8`), not the auth flow itself. Its effect mounts the instant `user` becomes truthy after a successful login and immediately runs `checkIdle()` against whatever `farmer-app-last-active-at` timestamp was already sitting in `localStorage`. If that timestamp was left over from a session that ended over an hour earlier (the common case on TestFlight, where the app sits closed for hours between opens), the brand-new login got force-signed-out on the spot — and since the check-fails path deliberately skips refreshing the timestamp (so it can still catch a genuine "app reopened after hours idle while still logged in" case), the stale value never updated, so *every* subsequent login attempt hit the same stale check and looped forever. Fixed by resetting the idle clock (`markIdleActivity()` in `useIdleLogout.js`) on the Supabase `SIGNED_IN` auth event specifically — that event fires only for an explicit interactive login, never for cold-start session restoration (`INITIAL_SESSION`), so a fresh login is never judged against a pre-login timestamp while the genuine idle-resume case is untouched.
