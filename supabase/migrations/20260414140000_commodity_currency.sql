-- Add currency column to commodities table
-- Allows admin to set EUR, RON, or USD per commodity

ALTER TABLE public.commodities
  ADD COLUMN IF NOT EXISTS currency text NOT NULL DEFAULT 'EUR';

-- Seed sunflower with USD (existing convention)
UPDATE public.commodities
  SET currency = 'USD'
 WHERE name ILIKE '%floarea soarelui%';

-- Update push-notify trigger to use stored currency instead of name inference
